...
class SQLBuilder implements ISqlBuilder{
	...
	@Override
	public String getFieldValue(Object value, boolean transferred) throws NotSupportDBType{
		if (value == null)
			return "null";
	...
	public void addWhere(String field, Operation operation, Object[] values) throws NotSupportDBType{
		addWhere(field, operation, values, true);
	}
	
	@Override
	public void addWhere(String field, Operation operation, Object[] values, boolean transferred) throws NotSupportDBType{
		switch (operation) {
		case otNotIn:
		case otIn:
			String vs = null;
			for (Object object : values) {
				if (vs == null)
					vs = getFieldValue(object, transferred);
				else{
					vs += "," + getFieldValue(object, transferred);
				}
			}
			...
			break;
		case otEqual:
			where.append(" " + field + " = " + getFieldValue(values[0], transferred));
			break;
		case otGreater:
			where.append(" " + field + " > " + getFieldValue(values[0], transferred));
			break;
		...
		case otLessAndEqual:
			...
			break;
		case otBetween:
			where.append(" " + field + " between " + getFieldValue(values[0], transferred) + " and " + getFieldValue(values[1], transferred));
			break;
		case otLike:
			where.append(" " + field + " like " + getSplitString() + getFieldValue(values[0], transferred) + getSplitString());
			break;
		case otUnequal:
			where.append(" " + field + " " + getUnEqualSymbolString() + " " + getFieldValue(values[0], transferred));
			break;
		case otNotNull:
			where.append(" " + field + " is not null");
			break;
		case otNull:
			where.append(" " + field + " is null");
			break;
		}
	}
	...
	public void addSet(String field, Object value) throws NotSupportDBType{
		addSet(field, value, true);
	}
	
	@Override
	public void addSet(String field, Object value, boolean transferred) throws NotSupportDBType{
		if (set.length() == 0)
			set.append(" " + field + " = " + getFieldValue(value, transferred));
		else
			set.append(" , " + field + " = " + getFieldValue(value, transferred));
	}
	...
	@Override
	public void addValue(Object value, boolean transferred) throws NotSupportDBType{
		if (value == null){
			value = "null";
		}
		
		if (values.length() == 0)
			values.append(getFieldValue(value, transferred));
		else
			values.append("," + getFieldValue(value, transferred));
	}
