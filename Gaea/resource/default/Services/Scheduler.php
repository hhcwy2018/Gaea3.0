...
class Scheduler
{
    ...
    public function getRoles()
    {
        ...
        foreach ($this->data[self::$PROCESS_ROOT_KEY] as $actionid => &$action) {
            $result[$actionid] = $action[self::$ACTION_ROLE_KEY];
        }
        return $result;
    }
    ...
    /**
     * 获取指定状态下所有操作的权限信息，可能返回空数组：[]
     * @param {string} 要获取权限的状态
     * @return 权限信息列表，格式：[{"name":"权限名称","id":"权限id"}]
     */
    public function getStateRoles($state)
    {
        //获取状态信息
        $info = $this->data[self::$STATE_ROOT_KEY][$state];
        if (empty($info[self::$ACTION_NODE_KEY])){
            return array();
        }

        $result = array();
        foreach($info[self::$ACTION_NODE_KEY] as $actionid){
            $roles = $this->roles($actionid);
            if (!empty($roles)){
                foreach($roles as $key=>$role){
                    $arr = array("id"=>$key,"name"=>$role);
                    $result[] = $arr;
                }
            }
        }
    ...
    public function getActionTasks($roles, $actionid, $start = null, $size = null)
    {
        ...
        $deny = true;
        foreach ($this->roles($actionid) as $key=> $role) {
            if (!empty($roles[$key])) {
                $deny = false;
                break;
            }
    ...
    public function __construct($userid, $taskid, $flowname = null, $uiid = null, $roles = null)
    {
        ...
        if (!empty($flowname)) {
            ...
            if (!empty($uiid)) {
                ...
                if (!empty($action[0])) {
                    foreach ($action as $a) {
                        foreach ($this->roles($a[self::$ACTION_ID_KEY]) as $key=> $role) {
                            if (!empty($roles[$key])) {
                                $this->actionid = $a["id"];
