    ...
    if (Tools.isNull(data) || data.ret != 0) {
        ...
        if (!Tools.isNull(griddata) && griddata.length > 0) {
            if (ggrid.getTotalCount() < total) {
                var addpage = total / ggrid.getPageSize() - total % ggrid.getPageSize() > 0 ? total % ggrid.getPageSize() + 1 : total / ggrid.getPageSize();
                ggrid.setTotalCount(total + addpage);
            }
            else
                ggrid.setTotalCount(data.data.params.total);
        } else {
