var business = {
    getParameterList: function (query) {
        ...
        var control = getFrameControlByName(uiInfo, "wlzbdcs_txtseachstype");
        setFrameControlValue(control, query.group_type);
        query.start = 0;
        query.size = 0;
    ...
    bomGYqzxSelect: function (id) {
        var grid = new TomcatDataGrid("bjsbcs_dggycs");
        var datas = grid.grid.getData();
        var selectData = [];
        var k = 0;
        for (var i = 0; i < datas.length; i++) {
            if (datas[i].param_id != id) {
                selectData[k] = {
                    id: datas[i].param_id,
                    text: datas[i].param_name,
                };
                k++;
            }
        }
        ...
        var uiInfo = globalGetUIInfo("bianjigongyicanshubom");
        var control = getFrameControlByName(uiInfo, "bjgycs_qzx");
...
function setProcessData(listname, gridData, key, pagename) {
    if (!pagename) {
        pagename = "bianjigongyicanshu";
    }
    GlobalDialog.showFrameDialog(pagename, "编辑工艺参数", function (b, uiInfo) {
        ...
        control = getFrameControlByName(uiInfo, "bjgycs_txtsbdjdygycs");
        setFrameControlValue(control, gridData.gycsbz_sbdjcs);

        control = getFrameControlByName(uiInfo, "bjgycs_ywz");
        if (control) {
            setFrameControlValue(control, gridData.gycsbz_servicegroup);
        }
        control = getFrameControlByName(uiInfo, "bjgycs_qzx");
        if (control) {
            setFrameControlValue(control, gridData.gycsbz_preposition);
            business.bomGYqzxSelect(gridData.param_id);
        }
        ...
        ControlHelp.setEvent(uiInfo, "bjgycs_btnsave", "onClick", function () {
            ...
            if (key == undefined) {
                var pd = control.getSelecteds();
                var pd_text = control.getSelecteds();
                if (pd.length == 0) {
                    ...
                    if (pd != "") {
                        ...
                        pd = { id: pd, text: pd };
                        arr.push(pd);
                        pd = JSON.stringify(arr);
                    }
                    pd_text = pd;
                } else {
                    var arr = [];
                    arr.push(pd);
                    pd_text = JSON.stringify(arr);
                    pd = pd_text;
                }
            } else {
            ...
            control = getFrameControlByName(uiInfo, "bjgycs_ywz");
            if (control) {
                var ywz = getFrameControlValue(control); //业务组
            }

            control = getFrameControlByName(uiInfo, "bjgycs_qzx");
            if (control) {
                var qzx = getOptionSelected(control).id; //前置项
                var qzx_text = getOptionSelected(control).text; //前置项
            }
            control = getFrameControlByName(uiInfo, "bjgycs_txtsbdjdygycs");
            if (control) {
                var djcs = getFrameControlValue(control); //对接参数
            }
            ...
            var rowData = {
                ...
                "gycsbz_fs": srfs_text,
                "gycsbz_fs_id": srfs,
                "gycsbz_servicegroup": ywz,
                "gycsbz_preposition": qzx,
                "gycsbz_preposition_text": qzx_text,
                "gycsbz_sbdjcs": djcs,
