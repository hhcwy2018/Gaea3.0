...
import com.wh.gaea.control.grid.ColorTableRowRender;
import com.wh.gaea.control.grid.JsonArrayTableModel;
import com.wh.swing.tools.MsgHelper;
import com.wh.tools.JsonHelp;

public class RequirementTableControl {
	...
	public void loadRow() {
		...
		closeTime.setFormats(new SimpleDateFormat(RequirementDefines.DATE_FORMAT));
		planEndTime.setLocale(Locale.CHINA);
		planEndTime.setFormats(new SimpleDateFormat(RequirementDefines.DATE_FORMAT));
		useTime.setLocale(Locale.CHINA);
		useTime.setFormats(new SimpleDateFormat(RequirementDefines.DATE_FORMAT));
...
//			model.setValueAt(closeTime.getDate(), row, RequirementDefines.field_close_time);
	...
	public void setData(JSONArray data, boolean isView, boolean filter) throws Exception {
		JsonArrayTableModel tableModel = new JsonArrayTableModel(
				isView ? RequirementDefines.View_ColumnNames : RequirementDefines.ColumnNames);
	 ...
	 * 将行数据加入依赖树对象
	 * 
	 * @param rows 依赖树对象，格式：{"id":{id, ...,child:{id:{}}}}
	 * @param row  行数据
	 * @param key  行中依赖项目的key
	 */
	protected void refreshDependTree(JSONObject rows, JSONObject row, JSONObject depends, String key) {
		...
		if (row.has(key) && row.get(key) instanceof JSONArray && row.getJSONArray(key).length() > 0) {
			...
			for (Object object : ids) {
				...
				if (!id.isEmpty()) {
					if (rows.has(id) && depends.has(mid))
						refreshDependTree(rows, rows.getJSONObject(id),
								depends.getJSONObject(mid).getJSONObject("child"), key);
				}
			}
		}
	}
	...
	public JSONObject remove() {
		...
		if (MsgHelper.showConfirmDialog("是否删除选定的记录？", "删除", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
			JsonArrayTableModel model = (JsonArrayTableModel) table.getModel();
