...
import com.wh.gaea.editor.ModelflowSelectDialog;
import com.wh.gaea.editor.ModelflowSelectDialog.Result;
...
public class ViewControl {
		...
		if (rows != null) {
			if (!allowSameVersion) {
				if (rowdata.getString(RequirementDefines.field_version)
					...
					MsgHelper.showMessage("此需求与当前选定需求版本一致，不能设置！");
					return false;
				}
			}
			for (Object object : rows) {
				if (object.equals(id)) {
					MsgHelper.showMessage("此需求已经依赖当前选定需求，不能循环依赖！");
					return false;
				}
			}
		}
	...
	public RequirementTableControl tableControl = new RequirementTableControl();
	...
	protected void setColorEnd(JButton button, FormType ft, String key) throws Exception {
		...
		case dtNormal: {
			...
			if (colorSelectRow == -1) {
				MsgHelper.showMessage("请先选择一个需求！");
				return;
			}
	...
	public void save() throws Exception {
		...
		if (requirementFile.exists()) {
			...
			JSONArray data = ((JSONObject) JsonHelp.parseCacheJson(requirementFile, null))
	...
	public void export(String version, String typeName, JSONArray data) {
		File saveFile = SwingTools.selectSaveFile(null, null, "导出数据【" + version + "】", "excel文件=xlsx");
		if (saveFile == null)
			return;
	...
	public void addRequirement() {
		String version = tableControl.getInputMainVersion();
		if (version == null || version.isEmpty()) {
			MsgHelper.showMessage("主版本号不能为空，请输入后再试！");
