...
import com.wh.gaea.control.grid.IconTableCellRender;
...
import com.wh.gaea.draws.drawinfo.DrawInfoDefines;
...
import com.wh.swing.tools.list.checkbox.CheckBoxListRenderer;
...
import com.wh.swing.tools.tree.checknode.ICheck;
...
public class CSSEditor extends ChildForm implements IMainMenuOperation {
	...
	public CSSEditor(IMainControl mainControl) {
		...
		button_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				String name = MsgHelper.showInputDialog("请输入模板名称");
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (MsgHelper.showConfirmDialog("是否删除选定的模板？", JOptionPane.YES_NO_OPTION) == JOptionPane.NO_OPTION)
					return;
		...
		button_5.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (MsgHelper.showConfirmDialog("保存模板将覆盖已经存在的CSS样式，是否继续？", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION)
					return;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (newTemplate(name)){
					...
					try {
						...
						e1.printStackTrace();
						MsgHelper.showMessage(null, "拷贝文件：" + source.getAbsolutePath() + " 到 " + dest.getAbsolutePath() + "失败！", "复制失败");
					}
				}
			}
		});
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (MsgHelper.showConfirmDialog("应用模板将使用选定模板样式覆盖当前项目所有UI的CSS样式，是否继续？", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION)
					return;
				
				if (templates.getSelectedItem() != null){
					if (applyTemplate(templates.getSelectedItem().toString()))
						MsgHelper.showMessage("应用成功！");
		...
		btncss_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (MsgHelper.showConfirmDialog("保存样式将覆盖选定控件已经存在的CSS样式，是否继续？", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION)
					return;
					...
					try {
						if (info.data.getString(KEY_TYPE).compareToIgnoreCase(TreeItemObject_Control) == 0){
							if (isEdit && e.getOldLeadSelectionPath() != null){
								if (MsgHelper.showConfirmDialog("已经更改数据，是否保存？", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION){
		...
		for (String filename : nodes.keySet()) {
			try {
				...
				for (String id: editControls.keySet()) {
					IUINode uiNode = (IUINode)canvas.getNode(id);
					if (uiNode == null)
						continue;
	...
	private boolean applyTemplate(String templateName){
		...
		if (!templateFile.exists()){
			MsgHelper.showMessage(null, "模板文件：" + templateFile.getAbsolutePath() + "不存在！", "应用模板失败");
			return false;
		}
	...
	private void extractTemplate(){
		...
		if (model.getRowCount() == 0){
			MsgHelper.showMessage("请先新建/选择一个模板后再试！");
			return;
		}
		try {
	...
	private void selectTemplate(String templateName){
		...
		JSONObject datas;
		try {
			datas = (JSONObject) JsonHelp.parseCacheJson(templateFile, null);
			JSONArray names = datas.names();
			for (int i = 0; i < names.length(); i++) {
				String name = names.getString(i);
				JSONObject info = datas.getJSONObject(name);
				if (controls.containsKey(name)){
					info.put(IDrawInfoDefines.Full_TypeName_Key, controls.get(name).getString(IDrawInfoDefines.Full_TypeName_Key));
...
//			column.setCellEditor(new TemplateTableCellEditor(new JTextField()));
	...
	private void saveTemplate(String templateName){
		...
		if (templateFile.exists()){
			if (!templateFile.delete()){
				MsgHelper.showMessage(null, "不能删除模板文件：" + templateFile.getAbsolutePath(), "保存模板失败");
				return;
			}
		}
	...
	private boolean newTemplate(String templateName){
		File templateFile = getTemplateFile(templateName);
		if (templateFile.exists()){
			if (MsgHelper.showConfirmDialog("要建立的模板已经存在，是否覆盖？", JOptionPane.YES_NO_OPTION) == JOptionPane.NO_OPTION)
				return false;
			
			if (!templateFile.delete()){
				MsgHelper.showMessage(null, "删除原模板文件：" + templateFile.getAbsolutePath() + "失败！", "建立模板失败");
				return false;
			}
		}
	...
	public void save(DefaultMutableTreeNode node){
		...
		try {
			...
			IUINode uiNode = (IUINode)canvas.getNode(info.data.getString(TreeHelp.ID_KEY));
			DrawInfo drawInfo = uiNode.getDrawInfo();
	...
	public void loadTree(){
		HashMap<String, IDrawNode> uiWorkflowNodes = new HashMap<>();
		...
		try {
			EditorEnvironment.traverseModel(new ITraverseDrawNode() {
				...
				public boolean onNode(File file, String title, IDrawNode node, Object param) {
					String id = EditorEnvironment.getUIID(node.getId());
				...
				public boolean onNode(File file, String title, IDrawNode node, Object param) {
					...
					try {
						...
						if (uiWorkflowNodes.containsKey(id)){
							workflownode = (DrawNode) uiWorkflowNodes.get(id);
						...
						for (IDrawNode drawNode : canvas.getNodes()) {
							...
							subInfo.put(TreeHelp.ID_KEY, drawNode.getId());
