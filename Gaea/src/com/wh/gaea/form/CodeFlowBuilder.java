...
import com.wh.gaea.control.ParseApp;
...
import com.wh.gaea.draws.AppWorkflowNode;
import com.wh.gaea.draws.AppWorkflowNode.BeginNode;
import com.wh.gaea.draws.AppWorkflowNode.ChildAppWorkflowNode;
...
import com.wh.gaea.draws.AppWorkflowNode.PhpCommandNode;
...
public class CodeFlowBuilder extends ChildForm implements IMainMenuOperation, IControl{
	...
	protected void onEditNode(IDrawNode node) {
		if (node instanceof ChildAppWorkflowNode){
			mainControl.openCodeflowRelation(node.getName(), uiid, workflowid);
			...
			try {
				...
				if (file.exists()){
					...
					MsgHelper.showMessage("文件：" + file.getAbsolutePath() + "不存在！");
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	protected void remove() {
		...
		if (MsgHelper.showConfirmDialog("是否删除选定的项目？", "删除", JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION)
			return;
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					MsgHelper.showMessage(null, "导出运行流程图成功！", "恭喜");
				} catch (Exception e2) {
					e2.printStackTrace();
		...
		button_4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if ((uiid == null || uiid.isEmpty()) && (workflowid == null || workflowid.isEmpty())){
					MsgHelper.showMessage("未关联任何界面及节点，无法生成！");
					return;
				}
