...
import com.wh.gaea.control.ControlTreeManager;
...
import com.wh.gaea.control.grid.design.DefaultPropertyClient.IUpdate;
...
import com.wh.gaea.draws.FlowNode;
import com.wh.gaea.draws.FlowNode.ANDNode;
import com.wh.gaea.draws.FlowNode.ActionNode;
...
import com.wh.gaea.draws.FlowNode.IFNode;
import com.wh.gaea.draws.FlowNode.LabelNode;
import com.wh.gaea.draws.FlowNode.ORNode;
...
import com.wh.gaea.draws.FlowNode.XORNode;
import com.wh.gaea.draws.RunFlowCanvas;
...
import com.wh.gaea.interfaces.IMainControl.IControl;
...
public class RunFlowBuilder extends ChildForm implements IMainMenuOperation, IControl {
	...
	protected static BeginNode findStartNode(IDrawCanvas canvas) {
	...
	protected static EndNode findEndNode(IDrawCanvas canvas) {
		for (IDrawNode node : canvas.getNodes()) {
			if (node instanceof EndNode)
				return (EndNode) node;
		}
	...
	protected void editDrawNode(DrawNode node) {
		...
		if (!allowEditNode(node)) {
			MsgHelper.showMessage(null, "流程图不支持编辑节点", "提示", JOptionPane.WARNING_MESSAGE);
		} else {
			onEditNode(node);
		}
	}
	...
	public RunFlowBuilder(IMainControl mainControl) {
		...
		editButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				DrawNode node = (DrawNode) canvas.getSelected();
		...
		btnFont.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (canvas.getSelected() == null) {
					MsgHelper.showMessage(null, "请先选择节点！");
					return;
				}

				Font font = ((DrawNode)canvas.getSelected()).font;

				Font result = JFontDialog.showDialog(RunFlowBuilder.this, "字体选择", true, font);
				if (result != null) {
					for (IDrawNode inode : canvas.getSelecteds()) {
	...
	public static class Publisher {
		...
		protected boolean processAction(FlowNode flowNode, JSONObject processInfo) throws Exception {
			if (flowNode instanceof ActionNode) {
				...
				if (actionNode.role == null || actionNode.role.size() == 0)
					throwException(actionNode, "动作节点的role必须设置！");
				...
				processInfo.put(ACTION_ROLE_KEY, actionNode.role == null ? null : actionNode.role.toJson());
			...
			if (parent == null) {
				...
				if (node instanceof ProcessNode) {
					if (parent instanceof ANDNode) {
						...
						IDrawNode stateNode = canvas.getNode(parent.getPrevs().get(0));
		...
		public void publish() throws Exception {
			if (EditorEnvironment.getPublishWebPath() == null) {
				MsgHelper.showMessage(null, "请先设置Web根目录！");
				return;
			}
			...
			if (beginNode == null) {
				MsgHelper.showMessage("必须设置起始节点，请添加起始节点并连线后重试！");
				return;
			}
			if (endNode == null) {
				MsgHelper.showMessage("必须设置结束节点，请添加结束节点并连线后重试！");
				return;
			}
			...
			for (IDrawNode node : canvas.getNodes()) {
				if (node instanceof StateNode) {
					...
					if (stateNode.state == null || stateNode.state.isEmpty())
						throwException(stateNode, "状态节点【" + node.getName() + "】未填写状态！");
					String key = stateNode.state.trim().toLowerCase();
					if (checkNodes.containsKey(key))
						throwException(stateNode, "状态节点【" + node.getName() + "】的状态【" + stateNode.state + "】已经存在！");
	...
	public void publish() {
		try {
			if (isEdit) {
				if (MsgHelper.showConfirmDialog("当前流程已经修改，是否保存后发布？",
						JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
					save(false);
				}
			}
			...
			published.publish();
			MsgHelper.showMessage(null, "导出运行流程图成功，关闭后打开目录！", "恭喜");
