...
error_reporting(0);
$require = CommonObject::parseRemoteData();

if (isset($require["data"])) {
    ...
    try {
        switch ($command) {
            ...
            case "gettaskrole":
                ...
                $scheduler = new Scheduler($userid, $taskid, $flowname, NULL, $role);
                $data = $scheduler->getTask($taskid);
                $result = array("ret" => false);;
                if (!empty($data) && !empty($data["state"])){
                    $state = $data["state"];
                    $data = $scheduler->getStateRoles($state);
                    $result = array("ret" => true, "data" => $data);
                }
                echo json_encode($result);    
                break;
            case "action":
                ...
                $data["role"] = $scheduler->getStateRoles($data["state"]);
                $result = array("ret" => true, "data" => $data);
                echo json_encode($result);
                break;
            case "getRoles":
