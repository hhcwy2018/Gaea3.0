...
var Tools = {
    ...
    /**
     * 转换Gaea设计器字体信息到浏览器格式的字体Style
     * @param {*} data 包含Gaea设计器格式的字体信息
     */
    convertGaeaFontToWebStyle: function (data) {
        var fontstyle = "";
    ...
    getDebugger: function () {
        if (Tools.isNull(Tools.isdebugger)) {
            Tools.isdebugger = Tools.getDebuggerInfo();
        }
        return Tools.isdebugger;
    },
    /**
    ...
    */
    printDiv: function (divs, paging, scale, headElements, mode, close, extraCss, keepAttr) {
        ...
        if (Tools.isNull(headElements)) {
            headElements = '<meta charset="utf-8">' +
                // '<meta name="viewport" content="width=device-width, initial-scale=96/300, user-scalable=no">' +
    ...
    */
    ajaxSubmit: function (json) {
        ...
        var ajaxError = function (jqXHR, textStatus, errorThrown) {
            ...
            if (!Tools.isNull(textStatus)) {
                ...
                if (textStatus == "parsererror") {
                    ...
                    if (bug == "true") {
                        alert("服务器错误！请检查相关配置！当前为测试状态不会重新刷新页面！");
                        result.err = errorThrown;
                    }else{
                        alert("服务器错误！请检查相关配置！确认后将退出登录！");
            ...
            Econsole.AjaxFinish(EconsoleAjaxId, result);
            if (bug == "false") {
                location.reload();
            }
            errorProc(result);
        ...
        var senddata = {
            ...
            success: function (data) {
                ...
                Econsole.AjaxFinish(EconsoleAjaxId, data);
                json.success(data);
            },
            error: ajaxError
        };
        ...
        Econsole.initAjax(senddata, EconsoleAjaxId);
    ...
    getDebuggerInfo: function () {
        var param = {
            keys: {},
            mark: "getDebuggerInfo"
        }
        var json = Tools.createAjaxPackage(param.mark, param.keys);
        json["Command_Running_Hint"] = "getDebuggerInfo" + name;
        Tools.updateAjaxPackageSign(json);
        var result = undefined;
        Tools.ajaxSimpleSubmit("./Services/ToolServices.php", false, json, function (data) {
            var info = mini.decode(data)
            if (info.ret)
                result = info["debugger"];
        });
        return result;
    },
    ...
    unique: function (array) {
        try {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                //否则的话把当前项push到临时数组里面
                if (newArray.indexOf(arr[i]) < 0) newArray.push(arr[i]);
                //indexOf 返回元素在result中的位置，如果没有返回-1；
            }
        }
        return newArray;
    },
    uploadLog: function (pageData, pageLen, reportData, reportLen) {
        $.ajax({
            url: "./Services/Console.php",
            async: true,
            type: "post",
            dataType: "text",
            data: {
    ...
    uploadScheduler: function (filename, data, flowdata) {
        Tools.ajaxSimpleSubmit("./Services/SchedulerUpload.php", false, {
            command:"run",
            filename: filename,
    ...
    loadFlowCharts:function(cellback){
        Tools.ajaxSimpleSubmit("./Services/SchedulerLoad.php", false, {
            command:"getFlowCharts",
        }, function (data) {
            data = JSON.parse(data);
            if (data.ret == 0) {
                cellback(data.data)
    ...
    loadFlowChart:function(filename,cellback){
        Tools.ajaxSimpleSubmit("./Services/SchedulerLoad.php", false, {
