...
function magicSetOnValidation(div, name, callback, systemcallback) {
    if (Tools.isNull(div))
        var control = mini.getByName(name);
    else
        var control = mini.getByName(name, div);
    ...
    if (Tools.isNull(control)) {
        if (Tools.isNull(div))
            ...
            $(div).find("[name='" + name + "']").change(function (e) {
                if (systemcallback) {
                    systemcallback(e);
                }
                callback(e.currentTarget);
        ...
        control.on("validation", function (e) {
            if (systemcallback) {
                systemcallback(e);
            }
            callback(e);
...
function magicSetOnKeyup(callback) {
    if (document.addEventListener) {
        ...
        document.attachEvent("onkeyup", callback);
    }
}
function magicSetOnPropertyChange(div, name, callback) {
    if (Tools.isNull(div))
        var control = mini.getByName(name);
    else
        var control = mini.getByName(name, div);
    $("[name='" + name + "']").bind("input propertychange", function (e) {
        callback(e, control)
    });
}

function defaultChangedEvent(e, control) {
    var data;
    var div;
    var uiData;
    var doEvent;
    ...
    if (Tools.isNull(control))
        return;
    controldata = control.options["userdata"];
    uiData = control.options["uidata"];
    magicDiv = control.options["magicdiv"];
    ...
    if (!Tools.isNull(magicDiv.Event.OnChanged)) {
        magicDiv.Event.OnChanged(data, magicDiv, controldata, uiData);
    }
}
function defaultValidationEvent(e) {
    var data;
    var uiData;
    var doEvent;
    var magicDiv;
    ...
    if (!Tools.isNull(e.value))
        data = e.value;

    if (!Tools.isNull(magicDiv.Event.OnValidation)) {
        magicDiv.Event.OnValidation(data, magicDiv, controldata, uiData)
    }
}
...
function MagicDiv() {
    this.Event = {
        ...
        OnChanged: undefined,//OnChanged(data,magicDiv,controldata, uiData)
        OnValidation: undefined,//OnValidation(data,magicDiv,controldata, uiData)
    }
    ...
    this.createColumn = function (dw, divWidth, divHeight, header, uiInfo, parent, needDiv) {
        ...
        switch (header.type) {
            ...
            case this.controlCommon.TEXTBOX_TYPE:
                ...
                var styleDivHeight = Tools.convetPX(divHeight, parseFloat(header.height) + 2);
                break;
            case this.controlCommon.TIME_TYPE:
            case this.controlCommon.BUTTON_TYPE:
                inputStyle += ";" + textFontStyle;
        ...
        if (!styleDivHeight) {
            styleDivHeight = styleHeight;
        }
        var style = "position:absolute;left:" + left + "px;top:" + top + "px;height:" + styleDivHeight + ";width:" +
        ...
        if (str != undefined) {
            ...
            switch (header.type) {
                case this.controlCommon.BUTTON_TYPE:
                    ...
                    var lineheightString = {
                        "line-height": parseInt(lineHeight) - 7 + "px"
                    };
            ...
            if (vtypeJson) {
                if (max) {
                    magicSetOnValidation(undefined, header.name, defaultValidationEvent, function (e) {
                        compareMax_fun(e, vtypeJson.name)
                    });
                    //str += this.setField('onvalidation', "compareMax_fun(e,'" + vtypeJson.name + "')");
                } else {
                    magicSetOnValidation(undefined, header.name, defaultValidationEvent, function (e) {
                        compare_fun(e, vtypeJson.name)
                    });
    ...
    this.relocation = function (replaceMode) {
        ...
        for (var index = 0; index < keys.length; index++) {
            ...
            if (parseInt(key) <= parseInt(this.header.placeIndex)) {
            ...
            var difference = Math.abs(curRect.size.width - prevRect.size.width);
            var posdiffer = Math.abs(prevRect.newLeft - curpos.left) < curRect.size.width ? curRect.size.width - Math.abs(prevRect.newLeft - curpos.left) + 10 : 0;
            if (difference > posdiffer)
                posdiffer = difference;
            $(cur).offset({
                left: curpos.left - posdiffer,
                top: curpos.top
            });
            prevRect = curRect;
            if (posdiffer > 0) {
                prevRect.left -= posdiffer;
            }
            prevRect.newLeft = $(cur).offset().left;
            if (!replaceMode) {
    ...
    this.getControl = function (dw, divWidth, divHeight, data, uiData, parent, userObj, proc) {
        ...
        switch (data.data.type) {
            ...
            case this.controlCommon.TEXTBOX_TYPE:
                ...
                magicSetOnValidation(div, data.data.name, defaultValidationEvent);
                magicSetOnPropertyChange(div, data.data.name, defaultChangedEvent);
            case this.controlCommon.IMAGE_TYPE:
        ...
        if (isMiniUI) {
            switch (data.data.type) {
                ...
                case this.controlCommon.PASSWORD_TYPE:
                case this.controlCommon.TIME_TYPE:
                    control.on("valuechanged", defaultChangedEvent);
                    break;
            }
        }
