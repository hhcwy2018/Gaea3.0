...
function ReportControlEx() {
    ...
    this.createProgressBarCell = function (header, textFontStyle, divWidth, divHeight) {
        var str = '<div ' +
            ...
            this.setField('style', Tools.addString(header.style,
                "position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:100%;line-height:" +
                Tools.convetPX(divHeight, header.reportHeight) + ";" + textFontStyle))+
            this.setField('class', Tools.insertString(" ", null, "processcontainer"));
            var num = parseFloat(header.start) > 100 ? 100 :parseFloat(header.start);
        if (true) {
            str +='>' +
            '<div id="' + header.id + '" class="processbar" style="width:' + num + '%;height:100%;">' + header.start + '%</div>' +
            '</div>';
            return str;
        }else{
            str +='>' +
            '<div id="' + header.id + '"  style="width:' + num + '%;height:100%;">' + header.start + '%</div>' +
            '</div>';
            return str;  
        }
    ...
    this.setCellValueByControl = function (control, value, controlType) {
        ...
        switch (controlType) {
            case this.controlCommon.IMAGE_TYPE:
                ...
                break;
            case this.controlCommon.ProgressBar_Type:
                var pboxc = control.childNodes[0]
                pboxc.style.width = value>100?"100%":value+"%";
                pboxc.innerText = value+"%";
                control.value = value;
                break;
            case this.controlCommon.CHECKBOX_TYPE:
            case this.controlCommon.RADIOBUTTONS_TYPE:
    ...
    this.createCell = function (allHeader, cellHeader, uiInfo, divWidth, divHeight, isEditor) {
        ...
        var textFontStyle = Tools.convertGaeaFontToWebStyle(data);
    ...
    this.getDataSourceId = function () {
        ...
        var result = [];
        for (var dsid in header.datasource) {
            result.push(dsid);
        }
