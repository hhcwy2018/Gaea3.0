...
import com.wh.swing.tools.dialog.AbstractFileSelector.DialogType;
import com.wh.swing.tools.dialog.AbstractFileSelector.Ext;
import com.wh.swing.tools.dialog.AbstractFileSelector.FileInfo;
...
public abstract class SwingTools {
	...
	static File DEFAULT_PATH = FileHelp.getRootPath();
	static INewFileSelector NEWFILESELECTOR = new INewFileSelector() {
	...
	public static boolean onlyInitDir = false;
	...
	public static void setup(INewFileSelector newFileSelector, File defaultPath) {
		NEWFILESELECTOR = newFileSelector;
		DEFAULT_PATH = defaultPath;
	}

	public static void onlyInitDir(boolean b) {
		onlyInitDir = b;
	}
	...
	public static File[] selectOpenAudioFiles(Component frame, String initDir, String initValue) {
		return selectOpenFiles(frame, initDir, initValue,
				"音频=midi;cda;rmf;aif;aiff;gsm;mid;mpa;swf;mp2;mp3;au;wav;ape;flac");
	}

	public static File[] selectOpenImageFiles(Component frame, String initDir, String initValue) {
		return selectOpenFiles(frame, initDir, initValue, "图片=png;gif;bmp");
	}

	public static File selectOpenAudioFile(Component frame, String initDir, String initValue) {
		return selectOpenFile(frame, initDir, initValue,
				"音频=midi;cda;rmf;aif;aiff;gsm;mid;mpa;swf;mp2;mp3;au;wav;ape;flac");
	}

	public static File selectOpenImageFile(Component frame, String initDir, String initValue) {
		return selectOpenFile(frame, initDir, initValue, "图片=png;gif;bmp");
	}

	public static File selectSaveImageFile(Component frame, String initDir, String initValue) {
		return selectSaveFile(frame, initDir, initValue, "图片=png;gif;bmp");
	}

	public static File[] selectOpenFiles(Component frame, String initDir, String initValue, String ext) {
		File[] files = openSelectFileDialog(FileDialog.LOAD, null, null, frame, getInitDir(initDir, lastOpenFile.file),
				initValue, ext, false, true);
		return files;
	}
	
	public static File selectOpenFile(String title, String msg, Component frame, String initDir, String initValue,
			String ext) {
		return selectFileDialog(FileDialog.LOAD, title, msg, frame, initDir, initValue, ext, false, lastOpenFile);
	}
	...
	static File[] openSelectFileDialog(int type, String title, String msg, Component frame, String initDir,
		...
		if (onlyDir) {
			...
			File dir = JDirChooser.showDialog(null, title, true,
					(initDir == null || initDir.isEmpty()) ? null : new File(initDir), msg);
		...
		List<FileInfo> result = AbstractFileSelector.show(dir, selectFile, title, dt, multipleSelect,
				extInfos.toArray(new Ext[extInfos.size()]), onlyInitDir, NEWFILESELECTOR);
	...
	public static void showErrorMessage(JComponent component, Exception e) {
		if (e == null) {
			MsgHelper.showMessage(component, "未知的错误！", "错误", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		e.printStackTrace();
		MsgHelper.showMessage(component, msg, "错误", JOptionPane.ERROR_MESSAGE);
	}
	...
	public static void showMaxDialog(JDialog frame) {
		Dimension screen = Toolkit.getDefaultToolkit().getScreenSize();
		Insets insets = Toolkit.getDefaultToolkit().getScreenInsets(frame.getGraphicsConfiguration());
		frame.setSize(screen.width, screen.height - insets.bottom);
		frame.setLocationRelativeTo(null);
