package com.wh.swing.tools.tree;
...
public class TreeHelp {
	...
	public static void expandOrCollapse(JTree tree, TreePath parent, boolean expand) {
		if (parent == null) {
			...
			parent = new TreePath(parentNode);
		}
		...
		final TreePath expandPath = parent;
		try {
			if (SwingUtilities.isEventDispatchThread()) {
				if (expand) {
					tree.expandPath(expandPath);
				} else {
					tree.collapsePath(expandPath);
				}
			} else
	...
	public static DefaultTreeModel newTreeModel(INewNode oNewNode) {
	...
	public static void renameTreeNode(JTree tree) {
		...
		if (!(node.getUserObject() instanceof TreeItemInfo) || name.compareTo("{}") == 0 || name.compareTo("[]") == 0) {
			MsgHelper.showMessage(null, "此节点不能重命名！", "编辑", JOptionPane.WARNING_MESSAGE);
			return;
		}

		name = MsgHelper.showInputDialog("请输入节点的新名字：", node.getUserObject().toString());
		...
		try {
			...
			e.printStackTrace();
			MsgHelper.showMessage(null, "编辑节点失败：" + e.getMessage() == null ? e.getClass().getName() : e.getMessage(),
					"编辑", JOptionPane.WARNING_MESSAGE);
		}
	...
	public static DefaultMutableTreeNode removeTreeNode(JTree tree, INewNode oNewNode) {
		...
		if (MsgHelper.showConfirmDialog("是否删除选定的节点？", "删除节点", JOptionPane.YES_NO_OPTION) == JOptionPane.NO_OPTION)
			return null;
		...
		if (selectNode == model.getRoot()) {
			tree.setModel(newTreeModel(oNewNode));
		} else
	...
	public static DefaultMutableTreeNode addTreeNode(JTree tree, String name, String idKey, String nameKey,
		...
		DefaultTreeModel model = (DefaultTreeModel) tree.getModel();
		return addTreeNode(tree, model, name, idKey, nameKey, pidKey, needEdit, oNewNode);
	}
	
	public static DefaultMutableTreeNode addTreeNode(JTree tree, DefaultTreeModel model, String name, String idKey, String nameKey,
			String pidKey, boolean needEdit, INewNode oNewNode) {
		...
		try {
			...
			e.printStackTrace();
			MsgHelper.showMessage(null, "添加节点失败：" + e.getMessage() == null ? e.getClass().getName() : e.getMessage(),
					"添加节点", JOptionPane.ERROR_MESSAGE);
			return null;
		}
	}
	...
	public static void jsonToTree(JTree tree, JSONArray valueObj, String idKey, String nameKey, String pidKey,
		...
		try {
			...
			DefaultTreeModel model = newTreeModel(onNewNode);
