		...
		void init(String pagename, HashMap<String, String> params, RequestType rt, NotifyType nt, IHttpResult hr) {
			if (params != null && params.size() > 0) {
				if (params.containsKey(SESSION))
					this.cookies = params.get(SESSION);
...
//					urlConnection.setRequestProperty("Cookie", request.cookies);

				urlConnection.connect();

				if (request.params != null) {
					BufferedWriter out = new BufferedWriter(
							new OutputStreamWriter(urlConnection.getOutputStream(), "UTF8"));
				...
				BufferedReader reader = new BufferedReader(
						new InputStreamReader(urlConnection.getInputStream(), "UTF8"));
	...
	public boolean AsyncGet(String url, HashMap<String, String> datas, IHttpResult hr) {
		requestManager
				.add(new RequestInfo(url, datas, RequestInfo.RequestType.rtGet, RequestInfo.NotifyType.ntThread, hr));
		return true;
	}
	...
	public boolean AsyncPost(String url, LinkedHashMap<String, String> datas, IHttpResult hr) {
		requestManager
				.add(new RequestInfo(url, datas, RequestInfo.RequestType.rtPost, RequestInfo.NotifyType.ntThread, hr));
		return true;
	}
	...
	public boolean SyncGet(String url, HashMap<String, String> datas, IHttpResult hr) {
		requestManager
				.add(new RequestInfo(url, datas, RequestInfo.RequestType.rtGet, RequestInfo.NotifyType.ntMain, hr));
		return true;
	}
	...
	public boolean SyncPost(String url, LinkedHashMap<String, String> datas, IHttpResult hr) {
		requestManager
				.add(new RequestInfo(url, datas, RequestInfo.RequestType.rtPost, RequestInfo.NotifyType.ntMain, hr));
		return true;
	}
	...
	public static String MD5(String value) {
		String result = MD5.encode(value).toLowerCase();
		return result;
	}
